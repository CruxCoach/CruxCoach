...
**Bestand und zu erhaltende Regeln:** Das Setup bietet zunächst eine optionale BLE-Verbindung und danach die Board-Auswahl. Ein erkannter Gerätename darf eine Board-Familie vorschlagen, aber weder andere Familien verstecken noch Layout, Größe oder Winkel behaupten. Die Katalogauswahl bleibt sichtbar; Downloads benötigen eine bewusste Auswahl beziehungsweise Zustimmung. Setup überspringen startet keine Tour und wählt nicht stillschweigend alle Kataloge aus. CruxCoach-Wiederherstellung gehört in den Import-Kontext neben den vorhandenen Importwegen.
...
Die Board-Auswahl zeigt eine kompakte, aber vollständige Identifikation: Familie, Modell beziehungsweise Layout sowie die relevante physische Größe/Konfiguration. Im geöffneten Picker darf diese Zusammenfassung umbrechen und wird mit einer kleinen Vorschau kombiniert. Eine BLE-Verbindung allein ist kein Nachweis für die montierten Griffe oder die gewählte Größe. Winkel verwenden die Fähigkeiten des ausgewählten Boards; Filter zurücksetzen verändert den Winkel nicht.
...
| --- | --- |
| Setup und Tour | Download-Zustimmung, Überspringen ohne Nebenwirkung, BLE ohne Hardware, tatsächliche Zielaktionen, gesperrte Hintergrundgesten und Zwischenräume, Neustart und gespeicherte Eintrags-UUID. Ausgangspunkte: `BrowserUxTest`, `BrowserTourTest`, `OnboardingDownloadSelectionTest`. |
