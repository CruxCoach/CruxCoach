...
class ApkInstallStatusReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val deferUserConfirmation = intent.getBooleanExtra(
            ApkInstaller.EXTRA_DEFER_USER_CONFIRMATION,
            false,
        )
        ...
        when (status) {
            PackageInstaller.STATUS_PENDING_USER_ACTION -> {
                ...
                if (userAction != null) {
                    ...
                    repository.onConsentRequired(userAction, deferUserConfirmation) {
                        pending.finish()
                    }
                } else {
