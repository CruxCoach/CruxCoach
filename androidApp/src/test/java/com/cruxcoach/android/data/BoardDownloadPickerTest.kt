...
class BoardDownloadPickerTest {
    @Test fun `first onboarding selects every single layout board without fetching even when downloads were previously enabled`() = runTest {
        ...
        every { repo.getDefaultLayoutForBrand(any()) } returns null
        val selector = AuroraBoardSelector(prefs, sync, repo)
        for ((brand, ids) in BoardConstants.AURORA_SINGLE_LAYOUT_DEFAULTS) {
            selector.select(brand, allowDownload = false)
            assertEquals(brand.wireValue, prefs.boardBrand.first())
            assertEquals(ids.first, prefs.boardLayoutId.first())
            assertEquals(ids.second, prefs.boardProductSizeId.first())
        }
        selector.select(BoardBrand.GRASSHOPPER, productSizeId = 6, allowDownload = false)
        assertEquals(6, prefs.boardProductSizeId.first())
