...
- First candidate render `f7892565` / VersionCode `1000019` passed the eight Compact semantic checks (EN/DE, Light/Dark, 1.0/1.5): both named actions are 126×126 px at 420 dpi and selection is checked. Visual review rejected round 1 because the old single-row metadata/outcome columns squeezed the date and name at 1.5 font scale. The bounded correction keeps the action layout but lets metadata and outcome wrap as whole items inside the primary content column; a second artifact-bound render is required.
...
- Paging failures retain every loaded row and expose a localized inline retry at the list end instead of silently disappearing. Retrying clears the transient error before the request and on success. `logbook/page-error` is the deterministic supplemental fixture; device rendering remains required.
