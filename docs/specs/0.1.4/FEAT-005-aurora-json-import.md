...
(`support@auroraclimbing.com`) and returns a single JSON file containing the
...
```kotlin
val intent = Intent(Intent.ACTION_SENDTO, "mailto:support@auroraclimbing.com".toUri())
...
- The "request your Aurora data" hint in the UI (§7.1) names the email
  address `support@auroraclimbing.com` exactly as Aurora has communicated
