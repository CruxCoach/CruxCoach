...
| **AC-45** nonces survive a restart | web | `competition-e2e.test.mjs` → *an intent keeps its nonce across a reload, so asking again replaces rather than adds*: one live registration on the relay after a second writer, and the documented in-memory fallback when storage is unavailable |
| **AC-44** no state without its exit | web + app | `competition-pages.test.mjs` → *every participant state a person can act on has the control to leave it*: a failed or expired payment stays payable, a withdrawal can be reversed while registration is open, and an accepted entrant can ask to be checked in |
...
| **AC-41** native paid entry | app + web | `CompetitionLightningTest` (23) → the shared Kotlin LNURL/BOLT11/zap layer against the same `vectors/zap.json` the website asserts on, including all six wrong-receipt cases; `CompetitionPaymentFlowTest` (7) → the app's own sequence over a TLS `MockWebServer`, with a deliberately unpayable invoice; `competition-pages.test.mjs` → *the two clients offer the same participant actions* |
