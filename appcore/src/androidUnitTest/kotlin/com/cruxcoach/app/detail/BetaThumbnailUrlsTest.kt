...
class BetaThumbnailUrlsTest {
    ...
    @Test fun ownedMirrorStillRequiresExactHostAndUnadornedHttpsHashUrl() {
        ...
        ).forEach { assertEquals(null, betaThumbnailHash(it), it) }
    ...
    @Test fun imageBytesMustMatchSignedHashBeforeCachingOrDisplay() {
        ...
        assertTrue(validBetaThumbnailBytes(bytes, hash, com.cruxcoach.app.testing.JvmHashing))
        kotlin.test.assertFalse(validBetaThumbnailBytes("tampered".toByteArray(), hash, com.cruxcoach.app.testing.JvmHashing))
        kotlin.test.assertFalse(validBetaThumbnailBytes(ByteArray(512 * 1024 + 1), hash, com.cruxcoach.app.testing.JvmHashing))
