...
/**
 * Port of Android `com.cruxcoach.android.ui.map.MapVenue`.
 *
 ...
 * room. Produced by [groupIntoVenues] from the per-board location rows so the
 * map renders one pin per place instead of several single-board markers
...
data class MapVenue(
     ...
     * Brand-level (not layout-level) because a venue can hold several layouts.
     */
    ...
    /** True when at least one Kilter Homewall is installed here. */
    val hasHomewall: Boolean
        get() = boards.any { it.boardBrand == BoardBrand.KILTER && it.layoutId == KILTER_HOMEWALL_LAYOUT }
}

/** Marker colour buckets — kept as a typed key so the renderer and any legend stay in lockstep. */
...
/**
 * ~11 m rounding (4 decimal places). Mirrors the cruxcoach.org map's `venueKey`
 * so the in-app map groups boards into venues the same way the website does —
 * two boards within ~11 m collapse to one pin.
 *
 * Intentionally tighter than the board picker's gym key (name + ~1 km): the map
 * clusters by precise geography (co-located boards = one pin), the picker groups
 * by gym identity. Different jobs, deliberately different granularity.
 */
...
// stays grouped. Used both to pick a venue's representative board and to order
// the boards list.
 ...
 * (Kilter > MoonBoard > other, then a non-blank name), since different brands at
 * the same gym often carry slightly different names. City and country are taken
 * from the first board that has a usable value (MoonBoard rows, for instance,
 * ship no country — `??` — so a co-located Kilter row fills it in).
 */
...
/**
 * The Pages snapshot owns venue identity and curation. Dynamic Blossom rows are
 * appended only when they describe a board installation not present in that
 * snapshot, so a newer upstream row can still appear without restoring a venue
 * the Pages exclusion/merge policy deliberately corrected.
 */
